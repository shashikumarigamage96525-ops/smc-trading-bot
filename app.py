...
# TRADE JOURNAL
# ============================================================
st.subheader("📔 Institutional Trade Journal")
...
with st.form("journal_form"):
    ...
    with j_col1:
        journal_asset = st.selectbox(
            ...
            symbols,
            index=(
                symbols.index(selected_coin)
                if selected_coin in symbols
                else 0
            ),
        )
        journal_type = st.selectbox("Type", ["LONG 🟢", "SHORT 🔴"])

    with j_col2:
        journal_entry = st.number_input(
            "Execution Price", value=float(entry_price), format="%.6f"
        )
        journal_exit = st.number_input(
            "Exit Price", value=float(tp1_price), format="%.6f"
        )

    with j_col3:
        journal_size = st.number_input(
            "Position Size (Units)", value=float(position_size), format="%.4f"
        )
    ...
    journal_notes = st.text_area(
        ...
        placeholder=(
            "Enter confluence factors, market context, or mental state..."
        ),
    )

    submit_journal = st.form_submit_button("💾 Save to Trade Journal")
