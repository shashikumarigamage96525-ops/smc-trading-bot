...
fig.add_hrect(
y0=entry_price0.998, y1=entry_price1.002,
...
with guide_col1:
st.markdown(f"🟢 Support Line:${supports[0]:,.4f}`" if supports else "🟢 **Support:** N/A") st.markdown(f"🔵 **EMA 50 Line:** `$`{ema_50:,.4f}`") st.markdown(f"🎯 **Entry Price:**${entry_price:,.4f}`")

with guide_col2:
st.markdown(f"🔴 Resistance Line:${resistances[-1]:,.4f}" if resistances else "🔴 **Resistance:** N/A") st.markdown(f"🟠 **EMA 200 Line:** ``${ema_200:,.4f}")
st.markdown(f"🛑 Stop Loss (SL):${sl_price:,.4f}`")

with guide_col3:
st.markdown(fvg_text)
st.markdown(f"⭐ VPVR POC Level:${poc_price:,.4f}`") st.markdown(f"💧 **Liquidity Pools:** `$`{bs_liq:,.4f}` /${ss_liq:,.4f}`")
...
st.markdown(f"- Entry:${entry_price:,.4f}`") st.markdown(f"- **SL:** `$`{sl_price:,.4f}`") st.markdown(f"- **TP1:**${tp1_price:,.4f}") st.markdown(f"- **TP2:** {tp3_price:,.4f}`")
